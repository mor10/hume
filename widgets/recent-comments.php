...
class hume_recent_comments extends WP_Widget {

	function __construct() {
        $widget_ops = array( 'classname' => 'widget_hume_recent_comments', 'description' => __('Displays recent comments with user avatar and excerpt.', 'hume') );
        parent::__construct( 'widget_hume_recent_comments', __('Enhanced Recent Comments','hume'), $widget_ops );
    }
	...
	function widget($args, $instance) {
		...
		if (!empty($widget_title)) {
			...
			echo $before_title . esc_html__( 'Recent Comments', 'hume' ) . $after_title;
		}
		?>

			<ul class="hume-widget-list">
						...
						foreach ( $comments as $comment ) { ?>
							...
							<li>
								<a href="<?php echo get_permalink($comment->comment_post_ID); ?>#comment-<?php echo $comment->comment_ID; ?>">
									...
									<p class="original-title"><span><?php _e('on','hume'); ?></span> <?php the_title_attribute( array( 'post' => $comment->comment_post_ID ) ); ?></p>
								</a>
							</li>
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php  _e('Title', 'hume'); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_comments'); ?>"><?php _e('Number of comments to display', 'hume'); ?>:
			...
			<small>(<?php _e('Defaults to 5 if empty','hume'); ?>)</small>
		</p>
...
register_widget('hume_recent_comments');
...
/*
function hume_comments_widget_registration() {
	unregister_widget('WP_Widget_Recent_Comments');
	register_widget('hume_recent_comments');
}
add_action('widgets_init', 'hume_comments_widget_registration');
*/